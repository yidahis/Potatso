...
extension TRuleType {
    ...
    public static func fromInt(_ intValue: Int) -> TRuleType? {
        switch intValue {
        case 1:
            return .Domain
        case 2:
...
extension TRuleType: CustomStringConvertible {
...
public final class Rule {
    ...
    public convenience init(str: String) throws {
        ...
        let value = parts[1]
        guard let type = TRuleType(rawValue: typeStr), let action = RuleAction(rawValue: actionStr), value.characters.count > 0 else {
            throw RuleError.invalidRule(str)
        }
        self.init(type: type, action: action, value: value)
    }
    
    public init(type: TRuleType, action: RuleAction, value: String) {
        self.type = type
        self.value = value
        self.action = action
    }
    ...
    public convenience init?(json: [String: AnyObject]) {
        guard let typeRaw = json["type"] as? String, let type = TRuleType(rawValue: typeRaw) else {
            return nil
        }
